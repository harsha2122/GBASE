...
if (!empty($equipment_interest))
    $lines[] = "Equipment Interest:   {$equipment_interest}";
if (!empty($business_type))
    $lines[] = "Type of Business:     {$business_type}";
if (!empty($production))
    $lines[] = "Production (tons/hr): {$production}";
if (!empty($referral))
    $lines[] = "How they found us:    {$referral}";

// Process-page specific fields
if (!empty($products))
    $lines[] = "Products:             {$products}";
if (!empty($cut_sizes))
    $lines[] = "Cut Sizes:            {$cut_sizes}";
if (!empty($capacities))
    $lines[] = "Capacities:           {$capacities}";
if (!empty($others_specify))
    $lines[] = "Others (Specify):     {$others_specify}";

// Multi-checkbox groups
if (!empty($product_types))
    $lines[] = "Product Types:        " . implode(', ', $product_types);
if (!empty($pre_process))
    $lines[] = "Pre-Process Steps:    " . implode(', ', $pre_process);
if (!empty($freezing_equipment))
    $lines[] = "Freezing Equipment:   " . implode(', ', $freezing_equipment);
if (!empty($heating_equipment))
    $lines[] = "Heating Equipment:    " . implode(', ', $heating_equipment);
